...
const App = () => {
  ...
  const [isLoad, setLoad] = useState(false); // подключение к серверу
  const [found, setFound] = useState(false); // ошибка подключения к серверу
  ...
  useEffect(() => {
    (async () => {
      ...
      setFound(true); // связь с сервером установлена
